...
export default function HeroShaderPlane() {
  ...
  useFrame((_, delta) => {
    ...
    // Smooth speed lerp
    smoothSpeed.current += (scrollState.speed - smoothSpeed.current) * 0.08;

    // Update uniforms directly (no React re-renders)
    materialRef.current.uniforms.uTime.value += delta;
    materialRef.current.uniforms.uMouse.value.set(
      smoothMouse.current.x,
      smoothMouse.current.y
    );
    materialRef.current.uniforms.uScrollSpeed.value = smoothSpeed.current;
